...
function renderDetailTable() {
    ...
    GLOBAL_ITEMS.forEach(row => {
        ...
        let rowStyle = '';
        if (row.excelColor) {
            // Prioridad: color de fondo del Excel
            rowStyle = `style="background-color:${row.excelColor}"`;
        } else if (txt.includes('REVERSA')) {
        ...
        const baseClass = 'hover:opacity-90 transition-colors font-semibold border-b border-gray-100';
        ...
        tbody.innerHTML += `<tr class="${rowClass}" ${rowStyle}><td class="pl-4 text-xs text-gray-600">${row.line}</td><td class="text-xs text-gray-600">${row.client}</td><td class="text-xs text-gray-600">${row.yarn}</td>${cellsHTML}</tr>`;
    });
