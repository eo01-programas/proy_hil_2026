...
function ingestData(jsonData, hiddenRows = new Set(), hiddenCols = new Set(), rowColors = new Map()) {
    ...
    for (let i = dataStartRow; i <= dataEndRow; i++) {
        ...
        // Si la fila no tiene línea ni cliente, y sus sumas son 0, es probable que sea un texto o subtítulo suelto. La ignoramos.
        if (Math.abs(rowSum) <= 0.01 && Math.abs(excelTotal) <= 0.01 && !rowContainsTOTAL && !lineVal && !clientVal) {
            continue;
        }
        ...
        const forbidden = ['RESERVA', 'REVERSA', 'CLIENTES VARIOS', 'CLIENTESVARIOS'];
