...
function renderCrudosTable() {
    ...
    crudoGroups.forEach((group, groupIndex) => {
        ...
        const filteredTotals = new Array(12).fill(0);
        filteredRows.forEach(r => {
            if (r.values) {
                activeIndices.forEach(idx => {
                    filteredTotals[idx] += r.values[idx] || 0;
                });
            }
        });
        const groupTotalSum = activeIndices.reduce((a, idx) => a + filteredTotals[idx], 0);
        if (Math.abs(groupTotalSum) <= 0.01) return;
        ...
        let reqCells = activeIndices.map(idx => {
            const rawVal = filteredTotals[idx] || 0;
        ...
        let totalReq = 0;
        activeIndices.forEach(idx => { totalReq += ((filteredTotals[idx] || 0) / defaultFactor); });
