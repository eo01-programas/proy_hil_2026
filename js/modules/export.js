...
function exportToExcel() {
    ...
    activeIndices.forEach(idx => {
        ...
        ws.push(["MAT. CRUDOS"]);
        crudoGroups.forEach(group => { const monthVal = group.columnTotals[idx]||0; if(Math.abs(monthVal)<0.0001) return; ws.push([`MATERIAL: ${cleanMaterialTitle(group.title)}`]); ws.push(["Línea","Cliente","Hilado",MONTH_NAMES[idx]]); group.rows.forEach(r=>{ ws.push([r.line,r.client,r.yarn, r.values[idx]||0]); }); ws.push(["","","TOTAL MES:", monthVal]); ws.push([]); });
        ws.push(["MAT. MEZCLAS"]);
        mezclaGroups.forEach(group => { const monthTotal = group.groupRawTotals[idx]||0; if(Math.abs(monthTotal)<0.0001) return; ws.push([`MATERIAL: ${cleanMaterialTitle(group.title)}`]); ws.push(["Línea","Cliente","Hilado",MONTH_NAMES[idx]]); (group.uniqueYarns?Array.from(group.uniqueYarns):[]).forEach(id=>{ const it = GLOBAL_ITEMS.find(x=>x.id===id); if(it) ws.push([it.line,it.client,it.yarn, it.values[idx]||0]); }); ws.push(["","","TOTAL MES:", monthTotal]); ws.push([]); });
